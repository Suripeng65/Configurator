<template>
  <div class="app-shell">
    ...
    <header class="top-bar">
      <span class="brand">Configurator</span>
      <span class="sep">|</span>
    ...
    <div class="app-body">
      ...
      <aside class="left-nav">
        ...
        <template v-if="isUITemplateRoute && route.params.dsId">
          <RouterLink :to="`/uitemplates/${route.params.dsId}/details`"     class="nav-sub" active-class="nav-sub-active">- Details</RouterLink>
          <RouterLink :to="`/uitemplates/${route.params.dsId}/layout`"      class="nav-sub" active-class="nav-sub-active">- Layout</RouterLink>
          <RouterLink :to="`/uitemplates/${route.params.dsId}/datasources`" class="nav-sub" active-class="nav-sub-active">- Datasources</RouterLink>
          <RouterLink :to="`/uitemplates/${route.params.dsId}/apis`"        class="nav-sub" active-class="nav-sub-active">- APIs</RouterLink>
          <RouterLink :to="`/uitemplates/${route.params.dsId}/instances`"   class="nav-sub" active-class="nav-sub-active">Instances</RouterLink>
        </template>
      </aside>
...
const isEditorRoute = computed(() =>
  route.path.startsWith('/uitemplates/') && !!route.params.dsId
)
...
const breadcrumb = computed(() => {
  const p = route.path
  if (p.startsWith('/uitemplates/') && route.params.dsId) {
    const section = route.meta.editorSection || 'layout'
    return `/ uitemplates / ${route.params.dsId} / ${section}`
  }
  if (p.startsWith('/variables/') && route.params.dsId) {
    return `/ variables / ${route.params.dsId}`
  }
  if (p === '/datasets')    return '/ datasets'
  if (p === '/variables')   return '/ variables'
  if (p === '/uitemplates') return '/ uitemplates'
  return p
})
