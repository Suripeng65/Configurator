...
var removeId = function (id) {
    var slot = idSlots.get(id);
    if (slot === undefined) {
        return false;
    }
    idSlots.delete(id);
    var state = prefixStates.get(slot.prefix);
    if (state !== undefined) {
...
var evalReadonly = function (uischema, data, path, ajv, config) {
    if (path === void 0) { path = undefined; }
    var fulfilled = isRuleFulfilled(uischema, data, path, ajv, config);
    switch (uischema.rule.effect) {
        case exports.RuleEffect.WRITABLE:
            return !fulfilled;
        case exports.RuleEffect.READONLY:
            return fulfilled;
        default:
            return false;
    }
...
var hasReadonlyRule = function (uischema) {
    if (uischema.rule &&
        (uischema.rule.effect === exports.RuleEffect.READONLY ||
            uischema.rule.effect === exports.RuleEffect.WRITABLE)) {
        return true;
    }
    return false;
};
...
var isReadonly = function (uischema, data, path, ajv, config) {
    if (path === void 0) { path = undefined; }
    if (uischema.rule) {
...
var generateUISchema = function (jsonSchema, schemaElements, currentRef, schemaName, layoutType, rootSchema) {
    ...
    var nonNullTypes = types.filter(function (t) { return t !== 'null'; });
    var effectiveTypes = nonNullTypes.length > 0 ? nonNullTypes : types;
...
var createTranslator = function (fn) {
    return function (id, defaultMessage, values) {
        var translation = fn(id, defaultMessage, values);
...
var defaultTranslator = createTranslator(function (_id, defaultMessage) { return defaultMessage; });
...
var isInherentlyEnabled = function (state, ownProps, uischema, schema, rootData, config) {
    var _a, _b, _c;
    if (!(config === null || config === void 0 ? void 0 : config.separateReadonlyFromDisabled) && ((_a = state === null || state === void 0 ? void 0 : state.jsonforms) === null || _a === void 0 ? void 0 : _a.readonly)) {
        return false;
    }
    ...
    if (!(config === null || config === void 0 ? void 0 : config.separateReadonlyFromDisabled)) {
...
var isInherentlyReadonly = function (state, ownProps, uischema, schema, rootData, config) {
    var _a, _b, _c;
    ...
    if (uischema && hasReadonlyRule(uischema)) {
        return isReadonly(uischema, rootData, ownProps === null || ownProps === void 0 ? void 0 : ownProps.path, getAjv(state), config);
    }
    if (typeof ((_b = uischema === null || uischema === void 0 ? void 0 : uischema.options) === null || _b === void 0 ? void 0 : _b.readonly) === 'boolean') {
        return uischema.options.readonly;
    }
    ...
    if (typeof (ownProps === null || ownProps === void 0 ? void 0 : ownProps.readonly) === 'boolean') {
...
var mapStateToControlProps = function (state, ownProps) {
    ...
    var readonly = isInherentlyReadonly(state, ownProps, uischema, resolvedSchema || rootSchema, rootData, config);
...
exports.evalReadonly = evalReadonly;
...
exports.hasReadonlyRule = hasReadonlyRule;
...
exports.isInherentlyReadonly = isInherentlyReadonly;
