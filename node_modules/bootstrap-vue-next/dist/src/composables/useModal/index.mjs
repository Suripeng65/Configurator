...
import { n as useSharedModalStack, t as BModal_default } from "../../../BModal-C-ZCau2E.mjs";
import "../../components/index.mjs";
import { computed, getCurrentInstance, inject, isReadonly, isRef, markRaw, onScopeDispose, shallowRef, toValue, watch } from "vue";
...
var useModal = () => {
	...
	const create = (obj = {}, options = {}) => {
		...
		promise.stop = watch(resolvedProps, (_newValue) => {
			...
			const v = {
				...
				_self,
				position: "modal",
				...previousIndex === -1 ? { _component: markRaw(BModal_default) } : store.value[previousIndex],
				options,
				promise
			};
	...
	*/
	...
	const get = (id) => {
		...
		if (registry?.value) {
			for (const [, modal] of registry?.value.entries() ?? []) if (toValue(modal.exposed?.id) === id) return {
				modal,
				show() {
					modal.exposed?.show();
				},
	...
	const current = () => {
		const findBModal = (component) => {
			...
			if (component.parent.type === BModal_default) return component.parent;
		...
		return {
			...
			modal: computed(() => modalComponent.value?.proxy)
		};
	};
	return {
		show,
...
//#endregion
export { useModal, useModalController };
