import { E as createSharedComposable, R as useToNumber, _ as useMouse$1, i as onClickOutside } from "./dist-Dk9C_HFS.mjs";
...
import { t as useShowHide } from "./useShowHide-CBFt1_EW.mjs";
import { t as ConditionalTeleport_default } from "./ConditionalTeleport-B4KYZIM6.mjs";
import { a as flip, d as size, i as autoUpdate, l as offset, n as useFloating, o as hide, r as autoPlacement, s as inline, t as arrow, u as shift } from "./floating-ui.vue-CH2o_mZU.mjs";
...
var BPopover_default = /* @__PURE__ */ defineComponent({
	inheritAttrs: false,
	__name: "BPopover",
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		const props = useDefaults(__props, "BPopover");
		const emit = __emit;
		...
		const arrow$1 = useTemplateRef("_arrow");
		...
		const floatingMiddleware = computed(() => {
			...
			const arr = [offset(props.offset !== null ? offsetNumber.value : props.tooltip ? 6 : 8)];
			if (!props.noFlip && !isAutoPlacement.value) arr.push(flip({
				boundary: boundary.value,
				rootBoundary: rootBoundary.value,
				padding: props.boundaryPadding
			}));
			if (isAutoPlacement.value) arr.push(autoPlacement({
				alignment: props.placement.split("-")[1] || void 0,
				boundary: boundary.value,
				rootBoundary: rootBoundary.value,
				padding: props.boundaryPadding
			}));
			if (!props.noShift) arr.push(shift({
				boundary: boundary.value,
				rootBoundary: rootBoundary.value,
				padding: props.boundaryPadding
			}));
			if (!props.noHide) arr.push(hide({
				boundary: boundary.value,
				rootBoundary: rootBoundary.value,
				padding: props.boundaryPadding
			}));
			if (props.inline) arr.push(inline());
			arr.push(arrow({
				element: arrow$1,
				padding: 10
			}));
			if (!props.noSize) arr.push(size({
				boundary: boundary.value,
				rootBoundary: rootBoundary.value,
				padding: props.boundaryPadding,
		...
		const { floatingStyles, middlewareData, placement, update } = useFloating(referenceElement, floatingElement, {
			placement: computed(() => isAutoPlacement.value ? void 0 : props.placement),
			middleware: floatingMiddleware,
			strategy: toRef(() => props.strategy)
		});
		const arrowStyle = ref({ position: "absolute" });
		let cleanup;
		const { showRef, hide: hide$1, show, toggle, throttleHide, computedNoAnimation, transitionProps, contentShowing, isVisible, isActive, renderRef, localTemporaryHide, setLocalTemporaryHide } = useShowHide(modelValue, props, emit, floatingElement, computedId, {
			showFn: () => {
				update();
				nextTick(() => {
					cleanup = autoUpdate(referenceElement.value, floatingElement.value, update, { animationFrame: props.realtime });
				});
			},
			hideFn: () => {
				if (cleanup) {
			...
			return openBlock(), createElementBlock(Fragment, null, [
				...
				createVNode(ConditionalTeleport_default, {
					...
					default: withCtx(() => [unref(renderRef) || unref(contentShowing) ? (openBlock(), createBlock(Transition, mergeProps({ key: 0 }, unref(transitionProps), { appear: modelValue.value || unref(props).visible }), {
						default: withCtx(() => [withDirectives(createElementVNode("div", mergeProps({ id: unref(computedId) }, unref(attrs), {
							ref: "_element",
						...
						}, [renderSlot(_ctx.$slots, "title", normalizeProps(guardReactiveProps(sharedSlots.value)), () => [createTextVNode(toDisplayString(unref(props).title), 1)])], 2)) : createCommentVNode("", true), unref(props).tooltip && !slots.title && !unref(props).title || !unref(props).tooltip ? (openBlock(), createElementBlock("div", {
							key: 1,
							class: normalizeClass([unref(props).tooltip ? "tooltip-inner" : "popover-body", unref(props).bodyClass])
						}, [renderSlot(_ctx.$slots, "default", normalizeProps(guardReactiveProps(sharedSlots.value)), () => [createTextVNode(toDisplayString(unref(props).body) + toDisplayString(unref(attrs).content), 1)])], 2)) : createCommentVNode("", true)], 4)], 16, _hoisted_2), [[vShow, unref(showRef) && !hidden.value]])]),
						_: 3
...
//# sourceMappingURL=floatingUi-62aeQtAb.mjs.map