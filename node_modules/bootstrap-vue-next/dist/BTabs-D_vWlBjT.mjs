import { C as tabsInjectionKey } from "./keys-DU7sQgl3.mjs";
...
var BTab_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		const showSlot = computed(() => computedActive.value || !computedLazy.value || computedLazy.value && !props.unmountLazy && lazyRenderCompleted.value);
...
var BTabs_default = /* @__PURE__ */ defineComponent({
	__name: "BTabs",
	props: /* @__PURE__ */ mergeModels({
	...
	setup(__props, { emit: __emit }) {
		const props = useDefaults(__props, "BTabs");
		const emit = __emit;
		const slots = useSlots();
		...
		function updateInitialIndexAndId() {
			...
			if (updateInitialActiveId) {
				...
				if (activeIndex.value > -1 && tabs.value[activeIndex.value]?.id) nextTick(() => {
					activeId.value = tabs.value[activeIndex.value]?.id;
		...
		const keynav = (e, direction) => {
			...
			nextTick(() => {
				if (activeIndex.value >= 0) getSafeDocument()?.getElementById(tabs.value[activeIndex.value]?.buttonId)?.focus();
			});
		};
		...
		watch(activeIndex, (newValue, oldValue) => {
			...
			if (tabEvent.defaultPrevented) {
				...
				nextTick(() => {
					if (prev >= 0) getSafeDocument()?.getElementById(tabs.value[prev]?.buttonId)?.focus();
				});
				return;
			}
		...
		const registerTab = (tab) => {
			...
			if (idx === -1) {
				...
				if (initialized) nextTick(() => {
					sortTabs();
				});
				else if (delayedTabSelection) nextTick(() => {
					updateInitialIndexAndId();
				});
			} else {
				...
				default: withCtx(() => [
					...
					createElementVNode("div", { class: normalizeClass([unref(props).navWrapperClass, {
						...
						(openBlock(true), createElementBlock(Fragment, null, renderList(tabs.value, (tab, idx) => {
							...
							}), [tab.titleComponent ? (openBlock(), createBlock(resolveDynamicComponent(tab.titleComponent), { key: 0 })) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [createTextVNode(toDisplayString(tab.title), 1)], 64))], 16, _hoisted_2)], 2);
						}), 128)),
...
//# sourceMappingURL=BTabs-D_vWlBjT.mjs.map