import { x as showHideRegistryKey } from "./keys-DU7sQgl3.mjs";
import { L as useThrottleFn, f as useEventListener } from "./dist-Dk9C_HFS.mjs";
import { r as BvTriggerableEvent } from "./classes-B0E5Y78Y.mjs";
import { t as noop } from "./functions-BcMenQqL.mjs";
import { computed, getCurrentInstance, inject, nextTick, onBeforeUnmount, onMounted, onUnmounted, readonly, ref, watch } from "vue";
...
var useShowHide = (modelValue, props, emit, element, computedId, options = {
	...
	const onBeforeEnter = [...[options.transitionProps?.onBeforeEnter ?? noop, props.transitionProps?.onBeforeEnter ?? noop].flat(), () => {
		isActive.value = true;
	}];
	const onEnter = [() => {
		requestAnimationFrame(() => {
	...
	const onAfterEnter = [
		...
		...[options.transitionProps?.onAfterEnter ?? noop, props.transitionProps?.onAfterEnter ?? noop].flat(),
		() => {
	...
	const onBeforeLeave = [
		...
		...[options.transitionProps?.onBeforeLeave ?? noop, props.transitionProps?.onBeforeLeave ?? noop].flat(),
		() => {
			trapActive.value = false;
		}
	];
...
//#endregion
export { useShowHide as t };

//# sourceMappingURL=useShowHide-CBFt1_EW.mjs.map