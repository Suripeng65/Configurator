...
function getAtPath(obj: any, path: Array<string | number>) {
...
export const useTemplateStore = defineStore('template', {
  state: () => ({
    ...
    isDirty: false,
    saveStatus: null as string | null,   // null | 'saving' | 'saved' | 'error'
  ...
  actions: {
    ...
    setValue(path: Array<string | number>, value: any) {
      if (!this.template) return
      if (path.length === 0) {
    ...
    renameKey(path: Array<string | number>, newKey: string) {
      if (!this.template) return
    ...
    deleteNode(path: Array<string | number>) {
      if (!this.template) return
      const parentPath = path.slice(0, -1)
      const parent = parentPath.length === 0
        ? this.template.layout
    ...
    addChild(path: Array<string | number>, key: string | number | null, type: string) {
      if (!this.template) return
    ...
    moveItem(parentPath: Array<string | number>, fromIndex: number, toIndex: number) {
      if (!this.template) return
    ...
    copyNode(sourcePath: Array<string | number>, destKey: string) {
      if (!this.template) return
    ...
    async fetchTemplateList() {
      try {
        const res = await axios.get(`/dataset-metadata/all`)
        if (res.data) {
    ...
    async fetchTemplate(id = 1) {
      try {
        const res = await axios.get(`/ui-template/${id}`)
    ...
    async fetchDeployment(id = 1) {
      try {
        const res = await axios.get(`/dataset-metadata-config/${id}`, {params: {dto: "full"}})
        res.data.isDirty = false
        res.data.saveStatus = null
        res.data.fetchError = null
