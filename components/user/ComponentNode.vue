      ...
      <template v-else>
        <div class="cn-fields">
          ...
          >
            ...
            <!-- number -->
            <BFormInput
              v-else-if="widgetKind(key) === 'number'"
              size="sm"
              type="number"
              :model-value="item[key]"
            ...
            <BFormTextarea
              v-else-if="widgetKind(key) === 'array-flat'"
              size="sm"
            ...
            <!-- object / unrecognized nested shape → JSON -->
            <BFormTextarea
              v-else-if="widgetKind(key) === 'object-json'"
              size="sm"
...
import { getOwnFieldKeys, fieldSchema, widgetKindFor, buildNodeFromSchema, groupedComponents } from '../AdaptComponents/schemaUtils'
...
// to inferring a widget from the runtime value when there's no schema match.
const componentDef = computed(() => ADAPT_COMPONENTS[props.item.component] ?? null)

function schemaFieldDef(key) {
  return fieldSchema(componentDef.value, key)
}

// ── JSONForms wiring for schema-known components ─────────────────────────────
// A self-contained schema scoped to this node's own editable properties (no
// $ref, so JSONForms' own internal ajv can validate it directly — no need to
// hand it the shared registry/$defs from AdaptComponents/index.ts).
...
function widgetKind(key) {
  const fs = schemaFieldDef(key)
  if (fs) return widgetKindFor(fs)
  const v = props.item[key]
  if (typeof v === 'boolean')             return 'boolean'
  if (typeof v === 'number')              return 'number'
  if (isFlatArray(v))                     return 'array-flat'
  if (v !== null && typeof v === 'object') return 'object-json'
  return 'string'
}
