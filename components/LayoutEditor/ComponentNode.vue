...
function setFieldJson(key, raw) { 
  try { setValue([...props.path, key], JSON.parse(raw)) } catch { /* ignore */ } 
}
...
function commitAddField() {
  ...
  if (newFieldType.value === 'number')       
    value = Number(newFieldValue.value)
  else if (newFieldType.value === 'boolean') 
    value = newFieldValue.value === 'true'
  else if (newFieldType.value === 'array')   
    value = newFieldValue.value ? newFieldValue.value.split(',').map(s => s.trim()) : []
  else                                       
...
<template>
  <div class="cn-node" :class="`depth-${depth}`">
    ...
    <div class="cn-header" @click="expanded = !expanded">
      ...
      <span class="cn-pill" @dblclick="changeComponent = true" v-if="!changeComponent">{{ item.component }}</span>
      <BFormSelect v-if="changeComponent" v-model="item.component" size="sm" class="cn-af-select" @change="changeComponent = false" @blur="changeComponent = false">
